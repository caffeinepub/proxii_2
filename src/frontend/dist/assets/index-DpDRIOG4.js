...
function TaskDetailModal({
  ...
  const [posterHistory, setPosterHistory] = reactExports.useState([]);
  ...
  reactExports.useEffect(() => {
    ...
    Promise.all([fetchUserById(task.user_id_origintor), fetchTaskHistory()]).then(([user, history]) => {
      setPosterInfo(user);
      setPosterHistory(
        history.filter(
  ...
  const ratings = posterHistory.map((h) => Number.parseFloat(h.rating_score)).filter((n) => !Number.isNaN(n));
            ...
            "div",
            {
              ...
              children: [
                ...
                successId === applicantId || selectedPerformer === applicantId || task.worker_id === applicantId ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                  ...
                  {
                    type: "button",
                    disabled: selectingId === applicantId || !!task.worker_id || !!selectedPerformer,
                    ...
                    style: {
                      ...
                      color: "oklch(0.72 0.15 295)",
                      background: "transparent",
                      cursor: selectingId === applicantId || !!task.worker_id || !!selectedPerformer ? "not-allowed" : "pointer",
                      opacity: !!task.worker_id || !!selectedPerformer ? 0.4 : 1
                    },
                    children: selectingId === applicantId ? "Selecting..." : "Select as Performer"
                  }
                )
              ]
            },
...
function ProfileScreen({
  ...
  reactExports.useEffect(() => {
    ...
    Promise.all([fetchTasks(), fetchTaskHistory()]).then(([tasks, history]) => {
      setAllTasks(tasks);
  ...
  const ratings = taskHistory.filter(
    (t) => t.worker_id === userId && t.rating_score && !Number.isNaN(Number.parseFloat(t.rating_score))
